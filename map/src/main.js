...
const LAYER_NOTES = {
  voronoi:
    "Each cell is colored by the engine's pick — <code>argmin(distance − weight)</code>. This is exactly how a clicked point resolves, identical to the /config wizard.",
  partition:
    "Nearest-seed partition generated by the build script, with manual overrides applied. Shown for comparison only."
};
...
let partitionLayer = null;
let seedLayer = null;
let effectivePartition = null;
...
function buildPartitionLayer() {
  return L.geoJSON(effectivePartition, {
    style(feature) {
      return {
    ...
    onEachFeature(feature, layer) {
      ...
      layer.bindTooltip(`${tag}: ${HIERARCHY[tag]?.label ?? feature.properties.label ?? tag}`);
    }
  });
}
...
function setLayer(name) {
  S.activeLayer = name;
  if (name === "voronoi") {
    if (partitionLayer) map.removeLayer(partitionLayer);
    if (voronoiLayer) voronoiLayer.addTo(map);
  } else {
    if (voronoiLayer) map.removeLayer(voronoiLayer);
    if (partitionLayer) partitionLayer.addTo(map);
  }
  // keep seeds on top
  if (seedLayer) seedLayer.bringToFront();
  el.layerNote.innerHTML = LAYER_NOTES[name];
  syncSegButtons(el.layerGroup, "layer", name);
  if (S.resolution) applyHighlight(S.resolution);
}
...
function applyHighlight(res) {
  ...
  if (partitionLayer && S.activeLayer === "partition") {
    partitionLayer.eachLayer((layer) => {
      const tag = layer.feature?.properties?.tag;
      const hot = localTags.has(tag);
      layer.setStyle({
        color: hot ? "#d53045" : colorForTag(tag),
        weight: hot ? 3 : 0.8,
        opacity: hot ? 0.95 : 0.5,
        fillOpacity: hot ? 0.22 : 0.18
      });
...
function wireControls() {
  ...
  el.layerGroup.querySelectorAll(".seg-btn").forEach((btn) => {
    const select = () => setLayer(btn.dataset.layer);
    btn.addEventListener("click", select);
    btn.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
...
async function loadMapData() {
  const [partitionRes, overridesRes] = await Promise.all([
    fetch("./data/zones/zones.partition.geojson"),
    fetch("./data/overrides/manual-overrides.geojson")
  ]);
  if (!partitionRes.ok || !overridesRes.ok) {
    throw new Error("Failed to load partition/override data.");
  }
  const partition = await partitionRes.json();
  const overrides = await overridesRes.json();
  effectivePartition = applyManualOverridesToPartition(partition, overrides);
}
...
async function init() {
  ...
  voronoiLayer = buildVoronoiLayer();
  partitionLayer = buildPartitionLayer();
